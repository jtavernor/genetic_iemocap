...
class IEMOCAPConfig:
    ...
    NUM_INPUTS = 10*40 # this is the size of the squashed input
    ...
    dataset = IEMOCAPDataset('../../data_providers/labels_dim40.pk', avg_audio_features=True)
    dataset.truncate(10) # truncate dataset to only include utterances of length <= 10 words
    dataset.train()
    subset_size = 100
    dataset.use_subset(subset_size)

    collater = CustomCollation(max_len=10)
    ...
    def fitness_fn(self, genome):
        ...
        logger.info(f'evaluated ind {self.cur_individual} f: {accuracy} size: {num_params}')
        self.cur_individual += 1
        if self.cur_individual == self.POPULATION_SIZE:
            # Generation complete
